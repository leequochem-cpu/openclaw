...
- Feishu/card actions: honor official `card.action.trigger` `open_chat_id` and resolve `chat_mode` before dispatch so WebSocket group button clicks no longer skip `groupPolicy` as DMs.

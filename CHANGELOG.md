...
- Feishu/reaction group gating: resolve DM vs group from the reacted message when official reaction events omit `chat_type`, so group reactions keep `groupPolicy` and no longer post DM pairing codes into group chats.

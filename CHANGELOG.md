...
- Discord/group DM interaction gating: apply `dm.groupEnabled` (default off) and `dm.groupChannels` to component clicks and native slash commands, matching message preflight so non-allowlisted Group DMs cannot run commands or inject into DM sessions.

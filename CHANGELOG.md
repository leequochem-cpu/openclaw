...
- Agents/session_status sandbox visibility: enforce the shared session-tree guard for sandboxed callers before reading status or writing model overrides, including `sessionId` lookups, so child sessions cannot inspect or mutate parent or sibling state.

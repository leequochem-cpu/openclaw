...
- Gateway/chat.abort after session rotation: persist abort partials to the run-scoped transcript instead of the current store entry, so `/new` or `/reset` during a streaming reply no longer writes leftover assistant text into the fresh session.

...
- Gateway/Control UI: refuse `config.set` / `config.apply` (and disable Save/Apply) when the on-disk config snapshot is invalid, matching `config.patch`, so a redacted empty Settings form cannot wipe channel tokens and auth secrets.

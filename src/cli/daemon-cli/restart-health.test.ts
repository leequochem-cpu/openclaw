...
describe("inspectGatewayRestart", () => {
  ...
  it("probes busy ports with unavailable listener details before marking runtime healthy", async () => {
    const service = {
      readRuntime: vi.fn(async () => ({ status: "running", pid: 8000 })),
    } as unknown as GatewayService;
    probeGateway.mockResolvedValue({
  ...
  it("does not mark attribution gaps healthy when the local gateway probe fails", async () => {
    const service = {
      readRuntime: vi.fn(async () => ({ status: "running", pid: 8000 })),
    } as unknown as GatewayService;

...
describe("inspectGatewayRestart", () => {
  ...
  it("probes busy ports with unavailable listener details before treating them as healthy", async () => {
    const service = {
      readRuntime: vi.fn(async () => ({ status: "running", pid: 8000 })),
    } as unknown as GatewayService;
  ...
  it("does not mark unavailable listener details healthy when the gateway probe fails", async () => {
    const service = {
      readRuntime: vi.fn(async () => ({ status: "running", pid: 8000 })),
    } as unknown as GatewayService;

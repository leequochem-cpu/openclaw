...
describe("zalouser monitor group mention gating", () => {
  ...
  it("blocks open-policy group messages when sender is not in groupAllowFrom", async () => {
    const { dispatchReplyWithBufferedBlockDispatcher } = installRuntime({
      commandAuthorized: false,
    });
    await __testing.processMessage({
  ...
  it("blocks open-policy group messages when sender is not in allowFrom fallback", async () => {
    const { dispatchReplyWithBufferedBlockDispatcher } = installRuntime({
      commandAuthorized: false,
    });
    await __testing.processMessage({
